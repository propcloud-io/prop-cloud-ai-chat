...
const App = () => {
  ...
  const startContinuousMonitoring = () => {
    ...
    const activities = ["📊 Analyzing market trends for March 18-20...", "👀 Monitoring competitor pricing changes...", "📱 Checking for new guest inquiries...", "🔍 Scanning for booking opportunities...", "💬 No new messages requiring attention", "📈 Market analysis complete - no immediate actions needed", "🎯 All systems monitoring actively..."];
    let activityIndex = 0;
  ...
  return <div className="min-h-screen bg-black text-white relative overflow-hidden">
      ...
      <div className="relative z-10">
        ...
        <header className="bg-black/90 backdrop-blur-xl border-b border-gray-800/50 p-4">
          <div className="flex items-center justify-between max-w-4xl mx-auto">
            ...
            <div className="flex items-center space-x-2">
              {conversationCompleted && <Button onClick={handleViewDashboard} className="bg-teal-600 hover:bg-teal-700 text-white">
                  View Dashboard
                </Button>}
              <Button variant="ghost" onClick={handleLogout} className="text-white hover:text-teal-600 transition-colors">
        ...
        <div className="max-w-5xl mx-auto p-6 h-[calc(100vh-80px)] flex flex-col">
          ...
          {propertyData && <div className="mb-4 animate-fade-in">
          ...
          <div className="flex-1 overflow-y-auto mb-6 space-y-6 pb-6 scroll-smooth">
            {messages.map(message => <div key={message.id} className="animate-fade-in">
                {message.type === 'notification' && <div className="flex justify-center mb-4">
                ...
                {message.type === 'status' && <div className="flex justify-center mb-4">
                ...
                {message.type === 'success' && <div className="flex justify-center mb-4">
                ...
                {message.type === 'guest' && <div className="flex items-start space-x-4 mb-6 hover:bg-gray-900/20 p-3 rounded-xl transition-all duration-200">
                ...
                {message.type === 'sent' && <div className="flex justify-end mb-6 hover:bg-gray-900/20 p-3 rounded-xl transition-all duration-200">
                    <div className="bg-gradient-to-br from-teal-600 to-teal-700 rounded-2xl p-5 max-w-3xl shadow-xl">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <button onClick={() => copyToClipboard(message.content)} className="text-teal-100 hover:text-white transition-colors p-2 rounded-lg hover:bg-teal-500/20" title="Copy message">
                ...
                {(message.type === 'user' || message.type === 'ai') && <div className={`flex ${message.type === 'user' ? 'justify-end' : 'justify-start'} mb-6`}>
                    <div className={`flex items-start space-x-4 max-w-4xl ${message.type === 'user' ? 'flex-row-reverse space-x-reverse' : ''} hover:bg-gray-900/20 p-3 rounded-xl transition-all duration-200`}>
                      ...
                      <Card className={`${message.type === 'ai' ? 'bg-gray-800/90' : 'bg-gradient-to-br from-teal-600/80 to-teal-700/80'} border-gray-700/50 backdrop-blur-sm relative group shadow-xl rounded-2xl`}>
                        <CardContent className="p-5">
                          {message.type === 'ai' && <button onClick={() => copyToClipboard(message.content)} className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity text-gray-400 hover:text-white p-2 rounded-lg hover:bg-gray-700/50" title="Copy response">
                              <Copy className="h-4 w-4" />
                            </button>}
          {message.content.includes("https://airbnb.com/rooms/12345678") && message.id === 1 ? <div className="text-white text-sm leading-relaxed">
              ...
              <button onClick={handleSampleLinkClick} className="mt-2 text-teal-400 hover:text-teal-300 underline transition-colors duration-200 cursor-pointer">
                https://airbnb.com/rooms/12345678
              </button>
            </div> : <p className="text-white text-sm leading-relaxed">{message.content}</p>}
            ...
            {showAiWorking && <div className="flex justify-center animate-fade-in">
                <div className="bg-gradient-to-r from-purple-900/30 to-blue-900/30 border border-purple-600/50 rounded-lg px-6 py-3 backdrop-blur-sm">
                  <div className="flex items-center space-x-3">
                    ...
                    <div className="flex space-x-1">
                      ...
                      <div className="w-1.5 h-1.5 bg-purple-400 rounded-full animate-pulse" style={{
                    animationDelay: '0.2s'
                  }}></div>
                      <div className="w-1.5 h-1.5 bg-purple-400 rounded-full animate-pulse" style={{
                    animationDelay: '0.4s'
                    ...
                    <CardContent className="p-4">
                      <div className="flex items-center space-x-3">
                        <div className="flex space-x-1">
                          ...
                          <div className="w-2 h-2 bg-teal-600 rounded-full animate-pulse" style={{
                        animationDelay: '0.2s'
                      }}></div>
                          <div className="w-2 h-2 bg-teal-600 rounded-full animate-pulse" style={{
                        animationDelay: '0.4s'
            ...
            {showMonitoring && <div className="mt-6 animate-fade-in">
                <div className="bg-gradient-to-r from-teal-900/30 to-blue-900/30 border border-teal-600/50 rounded-lg p-6 backdrop-blur-sm">
                  ...
                  <div className="space-y-3">
                    {monitoringActivity.map((activity, index) => <div key={index} className="flex items-center space-x-3 text-sm text-gray-300 animate-fade-in" style={{
            ...
            {showResponseOptions && <div className="bg-gray-900/90 border border-gray-700/50 rounded-lg p-4 backdrop-blur-sm">
                <h3 className="text-white font-medium mb-3 flex items-center justify-between">
                  ...
                  <button onClick={() => copyToClipboard(isEditing ? editedResponse : suggestedResponse)} className="text-gray-400 hover:text-white transition-colors p-1" title="Copy suggested response">
            ...
            {showBookingResponseOptions && <div className="bg-gray-900/90 border border-gray-700/50 rounded-lg p-4 backdrop-blur-sm animate-fade-in">
                <h3 className="text-white font-medium mb-3 flex items-center justify-between">
                  <div className="flex items-center">
                    <AlertCircle className="h-4 w-4 mr-2 text-teal-600" />
                    Suggested Booking Response:
                  </div>
                  <button onClick={() => copyToClipboard(isBookingEditing ? editedBookingResponse : bookingSuggestedResponse)} className="text-gray-400 hover:text-white transition-colors p-1" title="Copy suggested response">
                    <Copy className="h-3 w-3" />
                  </button>
                </h3>
                {isBookingEditing ? <div className="space-y-3">
                    <textarea value={editedBookingResponse} onChange={e => setEditedBookingResponse(e.target.value)} className="w-full bg-gray-800/70 border border-gray-600 rounded-lg p-3 text-white text-sm resize-none focus:border-teal-600 backdrop-blur-sm" rows={5} />
                    <div className="flex space-x-2">
                      <Button onClick={handleSendBookingResponse} className="bg-teal-600 hover:bg-teal-700 text-white">
                        <Send className="h-4 w-4 mr-2" />
                        Send Edited Response
                      </Button>
                      <Button onClick={() => setIsBookingEditing(false)} variant="outline" className="border-gray-600 text-gray-300 hover:bg-gray-800">
                        Cancel
                      </Button>
                    </div>
                  </div> : <div className="space-y-3">
                    ...
                    <div className="flex space-x-2">
                      ...
                      <Button onClick={handleEditBookingResponse} variant="outline" className="bg-gray-900 border-gray-600 text-white hover:bg-gray-800">
                        <Edit3 className="h-4 w-4 mr-2" />
                        Edit
                      </Button>
                    </div>
          ...
          {conversationStage === 'initial' && <Card className="bg-gray-900/90 border-gray-700/50 backdrop-blur-sm animate-fade-in sticky bottom-0">
              <CardContent className="p-4">
                <div className="flex space-x-2">
          ...
          {conversationCompleted && <div className="fixed bottom-6 right-6 z-50">
